...
class BCDDevice:
    ...
    def friendly(self, verbose=False):
        s = ''
        if self.type == 'boot':
            s += 'boot'
            ...
            if self.disk == 'partition':
                friendly = None
                if not verbose:
                    friendly = devicelocate.entry_to_friendly(self.gpt, 
                                    self.diskid, self.partid)
                if friendly:
                    s += friendly[0]
                else:
                    if self.gpt:
            ...
            if self.type == 'ramdisk' and self.optionsid:
                ...
                if (not verbose) and self.optionsid in knownguids:
                    optionsid = knownguids[self.optionsid][0]
                s += ',' + optionsid
